       ...
       [:<>
        ...
        (when (some? personalListHasDraftStatus)
          [grid {:container true :spacing 2}
           [grid {:item true}
            [button {:href (str "/quiz/" id "/list/" personalListId "/personal") :color "primary" :variant "contained"}
             (if personalListHasDraftStatus "Submit top 10" "View top 10")]]
           [grid {:item true}
            [back-to-overview-button]]])])]))

(defn quiz-page-container []
  [quiz-page @(rf/subscribe [::subs/loading-quiz?]) @(rf/subscribe [::subs/quiz]) @(rf/subscribe [::subs/number-of-participants]) @(rf/subscribe [::subs/quiz-lists])])
